...
void calcHitX() {
   steps = (int)(sqrt((Cy + 200) / HG) * 1000 / TIMER_PERIOD);
   boatHitX = boatX;
   boatHitSpd = boatSpd;
   for (int i = 0; i < steps; i++) {
      boatHitX += boatHitSpd;
      if (boatHitX > 340 || boatHitX < -210)
         boatHitSpd = -boatHitSpd;
   }
}
...
void display() {
   ...
   glColor3f(1, 0, 0);
   circle(boatHitX + 30, -200, 7);

   if (boatHitX + 5 < ballHitX  && ballHitX <= boatHitX + 55) {
      glColor3f(0, 1, 0);
      circle(winWidth / 2 - 20, winHeight / 2 - 20, 10);
   }
